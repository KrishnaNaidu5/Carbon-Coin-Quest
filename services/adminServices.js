...
//                totalFBT = OrderDetails.fbt_count;
...
//         // let finalSend = totalFBT.toString();
...
//         await blockchainServices.addTransaction(user_id, walletData._id, adminAddress, user_wallet_address, hash3, finalSend, '$FBT');
...
const VerifyOrder = async (req, res) => {
    ...
    OrderDetails.updateOne({ '_id': Order_id }, { $set: { 'payment_status': 'Paid' } }, { upsert: true }, function (err, result) {
        OrderDetails.findOne({ '_id': Order_id }).then(async OrderDetails => {
            ...
            console.log("user wallet", address, totalFBT)
            console.log("user_id", OrderDetails.user_id)
            let sendfbt = parseFloat(totalFBT);
            let finalSend = sendfbt.toString();
            // let finalSend = '153';
            console.log("address-", address, finalSend)
            let hashObject3 = await AdminCoinTransfer(address, finalSend);
            console.log(finalSend, '----------finalSend', typeof finalSend);
            let hash3 = hashObject3.transactionHash;
            await blockchainServices.addTransaction(user_id, Userwallet._id, adminAddress, address, hash3, finalSend, '$FBT');
...
const FBTSold = async (admin_bal) => {

    const total_FBT = 1000000000000000000000000000000000;
    const total = parseFloat(total_FBT) / Math.pow(10, 18);
    const fbtSold = total - admin_bal
    return fbtSold
}

const totalFBTRewardsDestributed = async (total_users_s) => {
